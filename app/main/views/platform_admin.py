...
def notifications_sent_by_service():
    ...
    if form.validate_on_submit():
        ...
        content_disposition = 'attachment; filename="{} to {} notification status per service report.csv"'.format(
            start_date, end_date
        )
        return (
            ...
            {
                "Content-Type": "text/csv; charset=utf-8",
                "Content-Disposition": f"{content_disposition}",
            },
        )
    ...
    if form.validate_on_submit():
        ...
        if rows:
            return (
                ...
                {
                    "Content-Type": "text/csv; charset=utf-8",
                    "Content-Disposition": f'attachment; filename="Daily SMS provider volumes report from {start_date} to {end_date}.csv"',  # noqa
                },
            )
        else:
            flash("No results for dates")
