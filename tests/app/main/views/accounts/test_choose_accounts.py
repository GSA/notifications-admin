...
def test_choose_account_should_show_choose_accounts_page(
    ...
    assert outer_list_items[0].a["href"] == url_for(
        ".organization_dashboard", org_id="o1"
    )
    ...
    assert outer_list_items[1].a["href"] == url_for(
        ".organization_dashboard", org_id="o2"
    )
    assert normalize_spaces(
    ...
    assert outer_list_items[2].a["href"] == url_for(
        ".organization_dashboard", org_id="o3"
    )
    assert normalize_spaces(
    ...
    assert outer_list_items[5].a["href"] == url_for(
        ".service_dashboard", service_id="12345"
    )
    assert outer_list_items[6].a.text == "service one (org 2)"
    assert outer_list_items[6].a["href"] == url_for(
        ".service_dashboard", service_id="12345"
    )
    assert outer_list_items[7].a.text == "service two (org 2)"
    assert outer_list_items[7].a["href"] == url_for(
        ".service_dashboard", service_id="67890"
    )
...
def test_choose_account_should_show_choose_accounts_page_if_no_services(
    ...
    mock_get_orgs_and_services.return_value = {"organizations": [], "services": []}
    resp = client_request.get("main.choose_account")
    page = resp.find("main", {"id": "main-content"})
...
def test_should_show_back_to_service_if_user_belongs_to_service(
    ...
    ).startswith(normalize_spaces(expected_page_text))
